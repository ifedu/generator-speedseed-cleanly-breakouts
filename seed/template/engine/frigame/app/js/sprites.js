...
(($, fg) => {
    ...
    G.setupBlocks = (data) => {
        let cornerX = fg.r.block1.width * 1.5
        let cornerY = fg.r.block1.height * 4
        let powerUpLocations = getLocations(data.bricks, data.powerUps)
        let powerDownLocations = getLocations(data.bricks, data.powerDowns)
        ...
        for (let row = 0; row < data.bricks.length; row += 1) {
            let row_data = data.bricks[row]

            for (let col = 0; col < row_data.length; col += 1) {
                let block = row_data[col]

                if (block) {
                    let block_name = [block, String(row), String(col)].join('_')
    ...
    G.addPowerdown = function (x, y) {
        let name = ['powerDown', String(powerUpCounter)].join('_')
    ...
    G.addPowerup = function (x, y) {
        let name = ['powerUp', String(powerUpCounter)].join('_')
    ...
    function countdownCallback() {
        ...
        if (this.userData.index >= 3) {
            ...
            $.each(G.balls, (name, ball) => ball.userData.active = true)

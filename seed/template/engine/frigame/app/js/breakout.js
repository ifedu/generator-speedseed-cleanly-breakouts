...
(($, fg) => {
    ...
    const SOUND_PATH = 'assets/resources/sfx'

    $(() => {
        const LOADING_BAR_WIDTH = $('#loadingBarB').width()
        ...
        fg.loadCallback((percent) => $('#loadingBar').width(LOADING_BAR_WIDTH * percent))

        // Start the game
        fg.startGame(() => {
            $('#welcomeScreen').fadeTo(1000, 0, () => $(this).remove())

            // Initialize the playground
            fg.playground().setBackground({background: 'bg_prerendered'})

 ...
 */
window.Level = function(levelNumber, world) {
    /**
...
Level.prototype.create = function() {
    ...
    const ball = new Ball(50, 250, this.game)
    ball.id = 1
    ...
    const level = this.currentLevel //lazy tipyng :)
    const rows = level.bricks.length

    for (let i = 0; i < rows; i++) {
        for (let c = 0; c < level.bricks[i].length; c++) {
            if (!level.bricks[i][c]) continue

            const brick = new Brick(this.initX+(this.brickWidth*c), this.initY+(this.brickHeight*i),level.bricks[i][c], this.game)
    ...
    // Adding the paddle
    const paddle = new Paddle((this.game.stage.canvas.width / 2), 368, this.game)
...
Level.prototype.getBounds = function() {
    return {
        lower: this.initY+ (this.brickHeight * this.currentLevel.bricks.length),
        upper:this.initY
    }
}

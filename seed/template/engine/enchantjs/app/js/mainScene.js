//main game scene
window.GameScene = Class.create(Scene, {
    initialize(lev) {
        ...
        const bottomLabel = new Label(`Lives: ${lives} Score: ${score} Level: ${level}`)
        ...
        window.gameID = document.getElementById('enchant-stage')
        window.gameID.addEventListener("mousemove", (e) => {
        ...
        //load sound
        this.brickDeath = game.assets['assets/sfx/brickDeath.mp3']

        bricks = []
        this.populateBricks()
        //add bricks to scene
        for (let i = 0, j = bricks.length; i < j; i++) {
        ...
        //countdown
        const numbers = new Sprite(32, 48)

        numbers.image = enchant.Game.instance.assets['assets/tiles.png']
        numbers.frame = 12
        numbers.x = this.width / 2
        numbers.y = this.height / 2
        numbers.countdownBlip = game.assets['assets/sfx/countdownBlip.mp3']
        numbers.recover = game.assets['assets/sfx/recover.mp3']
        //enchat.js timeline
        numbers.tl.cue({
            0() {
    ...
    update() {
        ...
        for (let i = 0, j = balls.length; i < j; i++) {
            balls[i].move()
            //collision check
            //walls
            if (balls[i].y > this.height) {
                ...
                if (balls.length < 1) {
                    if (!--lives) {
                        const gameover = new textScene('Game Over')
        ...
        if (bricks.length < 1) {
            ...
            const nextLevel = this.levels.length
                ? new GameScene(this.levels)
                : new textScene('You Win')
...
GameScene.prototype.populateBricks = function() {
    for (let i = 0, j = this.levels[0].bricks.length; i < j; i++) {
        for (let k = 0, l = this.levels[0].bricks[i].length; k < l; k++) {
            if (this.levels[0].bricks[i][k] !== X) {
                const newBrickX = k * 32
                const newBrickY = i * 16
                const newBrick = new Brick(newBrickX, newBrickY, this.levels[0].bricks[i][k])
...
GameScene.prototype.addPowers = function() {
    let powerUps = this.levels[0].powerUps
    let powerDowns = this.levels[0].powerDowns

    while (powerUps > 0) {
        const rand = Math.floor(Math.random() * bricks.length)

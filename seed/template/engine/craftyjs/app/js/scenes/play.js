Crafty.scene('play', () => {
    ...
    function _hasLocation(x, y, locations) {
        for (let i = 0; i < locations.length; ++i) {
            if (locations[i].x === x && locations[i].y === y) return true
        }
    ...
    function _populateLevel(level) {
        ...
        const setup = breakout.LevelSetups[level-1]
        const bricks = setup.bricks

        const cornerX = breakout.brick.WIDTH * 1.5
        const cornerY = breakout.brick.HEIGHT * 4

        const powerUpLocations = _getLocations(bricks, setup.powerUps)
        const powerDownLocations = _getLocations(bricks, setup.powerDowns)
        ...
        for (let y = 0; y < bricks.length; ++y) {
            for (let x = 0; x < bricks[y].length; ++x) {
                ...
                if (color) {
                    let bx = cornerX + x * breakout.brick.WIDTH
                    let by = cornerY + y * breakout.brick.HEIGHT

                    Crafty
                    .e(`2D, Canvas, Brick, ${color}`)
                    .attr({
                        x: bx,
                        y: by,
    ...
    .bind('MouseMove', (e) => _paddle.x = e.offsetX || e.layerX)
    //.textColor('#000000')

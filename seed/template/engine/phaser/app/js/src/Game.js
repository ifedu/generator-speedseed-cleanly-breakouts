...
BasicGame.Game.prototype = {
    ...
    render() {
        if (this.debug) {
            ...
            const debugBallRender = function (ball) {
    ...
    populateLevel(level) {
        ...
        const dropItemLimit = this.dropItemLimit + this.currentLevel
        const brickPartLimit = Math.floor(this.bricks.countLiving()/dropItemLimit)

        let brickStartLimit = 1
        let brickEndLimit = brickPartLimit

        for (let dropCount = 0; dropCount < dropItemLimit; dropCount++) {
            const randomBrick = this.getRandomInt(brickStartLimit,brickEndLimit)

            //Get random value in range
            const randomBrickName = "brick" + randomBrick
    ...
    createBall(active) {
        ...
        let tempBall = this.game.add.sprite(this.ballInitialX, this.ballInitialY, 'tiles')
        let tempCount = 0
    ...
    resetBalls() {
        ...
        const tempBall = this.balls.getFirstDead()

        if (tempBall) this.resetBall(tempBall)
    },
    ...
    takeOneLifeDown() {
        ...
        if (this.balls.countLiving() == 0) {
            ...
            const firstDeadBall = this.balls.getFirstDead()
    ...
    paddleUpdate() {
        ...
        this.paddle.frameName = (this.isPaddleNerfed)
            ? "paddle_small.png"
            : "paddle_big.png"
    ...
    paddleHitBallHandler(paddle, ball) {
        //just an empty handler
    },

    paddleHitBallProcess(paddle, ball) {
    ...
    determineBounceVelocityX(_paddle, _ball) {
        const bounceVelocityX = _ball.body.velocity.x

        const ballBodyCenterX = _ball.body.x + _ball.body.halfWidth
        const paddleBodyCenterX = _paddle.body.x + _paddle.body.halfWidth
        const distanceX = Math.abs(ballBodyCenterX - paddleBodyCenterX)

        if (_ball.body.right < _paddle.body.x) {
            //  Ball is on the left-hand vertical side of the paddle
            const directionVariable = (bounceVelocityX > 0) ? -1 : 1
        ...
        const ratio = (distanceX) / _paddle.body.halfWidth * bounceCoefficient

        return (this.ballSpeed * ratio)
    },

    determineBounceVelocityY(paddle, ball) {
        let bounceVelocityY = ball.body.velocity.y
    ...
    ballHitBrickHandler(_ball, _brick) {
        ...
        if (this.bricksWithItems.indexOf(_brick.name) > -1 ) this.dropItem(_brick.x, _brick.y)
    },

    ballHitBricklProcess(ball, _brick) {
        if (this.shakeEffect) this.shakeGame()  //funny extra )

        return true
    },

    dropItem(dropItemInitialX,dropItemInitialY) {
        let typeFrame = ""
        let itemEffectName = ""
        ...
        let dropItem = this.game.add.sprite(this.getRandomInt(32, this.game.world.width - 64), -32, 'tiles', typeFrame)
        let tempCount = 0

        if(this.items.countLiving() > 0) tempCount = this.items.countLiving()
    ...
    onAnimationCompleteBrick(sprite, animation) {
    ...
    paddleHitItemHandler(paddle, item) {
        //empty handler
    },

    paddleHitItemProcess(paddle, item) {
    ...
    ballHitBallHandler(ball1, ball2) {
        //empty handler
    },

    ballHitBallProcess(ball1, ball2) {
    ...
    shakeGame() {
        ...
        let rumbleSpeed = 50
        let rumbleInterval
        let rumbleStopTimeOut
        let rumbleTime = 500
        let rumbleDuration = 300 //in milliseconds
    ...
    shake(rect, x, y) {
        ...
        let rx = Math.floor(Math.random() * (x + 1)) - x / 2
        let ry = Math.floor(Math.random() * (y + 1)) - y / 2
